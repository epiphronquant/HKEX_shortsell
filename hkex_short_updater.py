...
### compare it with SFC most recent date, if it is the same then pass, it it is different then gather
header = {
...
r = r.text
sfc_date = pd.read_html(r)
sfc_date = sfc_date [0]
sfc_date = sfc_date ['Reporting Date']
sfc_date = sfc_date [0]
...
if date2 == sfc_date:
    ...
    date1 = sfc_date.strftime('%Y%m%d')
    date = sfc_date.strftime('%Y/%m/%d')
